...
$('#signin').click(function() {
        ...
        $.ajax({
            url : "form/login",
            type: 'POST',
            data : {'email':email,'upswd':upswd},
            success: function(resp) {

                if (resp == "success") {window.location.href = "dashboard";}
                else{
                    $('.form-results').html('<p class="alert alert-danger">' + resp + '</p>');
                }
            }
        });
        return false;
...
$('#sellerSearch').click(function() {
        var code  = $("#sellerCode").val();
        var by   = $("#searchBy").val();
        var text   = $("#searchText").val();

        $.ajax({
            url : "form/seller_search",
            type: 'POST',
            data : {'code':code,'by':by,'text':text},
            success: function(resp) {

                if (resp.status == "success") {
                	if(resp.data=="no_result"){
                ...
                $self.validate({
                    ...
                    $(form).ajaxSubmit({
                        ...
                        success: function(data) {
                            ...
                            if (data.result !== 'error') { window.location.href = "dashboard"; }
                        }
                    });
                    }
                });
