...
function getVideoPayload(source, embedTag) {
  if (!embedTag) {
    payload = '<div class="modal-body timModalBody videoModalBody">' +
  ...
  if (source == 'local') {
    payload = '<div class="modal-body timModalBody">' +
...
function timModal(options) {
  ...
  html = '<div id="timModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="message-modal" aria-hidden="true">';
  html += that.style;
  ...
  if (that.showHeader) {
    ...
    html += '<p class="h5" style="width: 100%; display:block">' + that.title + '</p>';
    html += '<a class="close" data-dismiss="modal">X</a>';
    html += '</div>';
  }
  if (that.type == 'general') {
    html += '<div class="modal-body timModalBody">';
    if (that.loadPage) {
      ...
      if (that.heading) {
        html += '<h2 class="modal-heading text-center tim-modal-heading">' + that.heading + '</h2>';
      }
  ...
  html += '</div>';
  if (that.showFooter) {
    html += '<div class="modal-footer timModalFooter">';
  ...
  $("#timModal").modal();
  if (that.loadPage) {
    $("#timModal .timModalBody").load(that.loadPage);
  }
  $('#timModal').on('hidden.bs.modal', function(e) {
    $(this).remove();
  });
}
...
//This function binds a video popup to everything that has classname timVideo
$('.timVideo').on('click', function() {
  var videoPopup = {};
