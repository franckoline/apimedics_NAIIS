$('#kycForm').submit(function(e){
    ...
    $('#checkDiagnose').submit(function(e){
        // var token = $('#token').val();
        var token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..pGzLborSnJSVmkNjLRcmOZ-OuiQrw8kEjwQiK7hVNJw";
        ...
        e.preventDefault();
          $.ajax({
             ...
             // async:false,
             success: function(data){
                 if(data==''){ $('#form-results').append('<p class="alert alert-warning"> No result found</p>');} else {
            ...
            for(i in data){
                 $('#form-results').append('<p> <div id="result-results"> </div> Name: '+data[i].Issue.Name+'</p><p> Accuracy: '+data[i].Issue.Accuracy+'</p><p> ICD: '+data[i].Issue.Icd+'</p><p> Icd Name: '+data[i].Issue.IcdName+'</p><p> Professional Name: '+data[i].Issue.ProfName+'</p><p> Ranking: '+data[i].Issue.Ranking+'</p><div class="btn-group" style=""><form><input type="hidden" id="pat_id" name="pat_id" value="'+patID+'"/> <input type="hidden" id="symptom_id" name="symptom_id" value="'+data[i].Issue.Id+'"/> <input type="hidden" id="issue_id" name="issue_id" value="'+data[i].Issue.Id+'"/></form> <form> <input type="hidden" id="diagnosis_result" name="diagnosis_result" value="valid"> <a href="#" onclick="invalidForm('+data[i]+'" class="btn btn-sm btn-success btn-inline">Valid </a> &nbsp;&nbsp;&nbsp;&nbsp; </form> <form id="invalidForm"'+data[i]+'"> <input type="hidden" id="diagnosis_result" name="diagnosis_result" value="invalid"><button type="submit" class="btn btn-inline btn-sm btn-danger">Invalid</button> </form></div><hr>');
